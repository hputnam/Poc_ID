...
chl_path <- "baseline_TPC_physio/data/TPC_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("baseline_TPC_physio/data/TPC_homogenate_vols/homogenate_vols.csv") %>%
  select(fragment_ID, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("baseline_TPC_physio/output/surface.area.calc.csv")
...
chl %>%
  select(fragment_ID, chla.ug.cm2, chlc2.ug.cm2) %>%
  mutate(timepoint="May2022")%>%
  filter(!is.na(chla.ug.cm2))%>%
  filter(!is.na(chlc2.ug.cm2))%>%
  write_csv(path = "baseline_TPC_physio/output/chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("baseline_TPC_physio/metadata_POC_TPC.csv")
chl <- right_join(chl, meta, by = "fragment_ID")
