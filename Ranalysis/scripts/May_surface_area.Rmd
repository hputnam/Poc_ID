...
#load wax data
wax.data <- read.csv("data/May_TPC_surface_area_data.csv", header=TRUE)
...
smpls%>%
  select(fragment_ID, surface.area.cm2)%>%
    write_csv("data/May_TPC_surface.area.calc.csv")
```

TPC Baseline Phys Fragment Data
#use wooden dowel standards to calculate surface area curve 
```{r}
...
smpls%>%
  select(fragment_ID, surface.area.cm2)%>%
    write_csv("data/May_TPC_Phys_surface.area.calc.csv")
```
