...
symcounts_plot <- sym_counts.sub.unique %>%
  ggplot(aes(x = factor(Timepoint, level = level_order), y = cells.cm2, color = mtORF, shape=mtORF, group = mtORF)) +
...
geom_text(x=2, y=1.0, label=paste("Species=",stat.labels$`Pr(>F)`[1]), size=3, color=ifelse(stat.labels$`Pr(>F)`[1] < 0.05, "black", "darkgray")) + 
  geom_text(x=2, y=0.95, label=paste("Time=",stat.labels$`Pr(>F)`[2]), size=3, color=ifelse(stat.labels$`Pr(>F)`[2] < 0.05, "black", "darkgray")) + 
  geom_text(x=2, y=0.90, label=paste("Species x Time=",stat.labels$`Pr(>F)`[3]), size=3, color=ifelse(stat.labels$`Pr(>F)`[3] < 0.05, "black", "darkgray")) +
