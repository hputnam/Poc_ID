---
title: "Surface Area Calculations from Wax Dipping for Heatwave timepoint 1"
...
summary(stnd.curve)$r.squared #r-squared = 0.99 20220326
...
smpls%>%
  select(fragment_ID, surface.area.cm2)%>%
    write_csv("data/surface.area.calc.csv")
```

TPC Baseline Fragment Data
#use wooden dowel standards to calculate surface area curve 
```{r}

#load wax data
wax.data <- read.csv("data/TPC_surface_area_data.csv", header=TRUE)
...
smpls%>%
  select(fragment_ID, surface.area.cm2)%>%
    write_csv("data/TPC_surface.area.calc.csv")
```
