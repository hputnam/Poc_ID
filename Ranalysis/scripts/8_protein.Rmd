...
prot_plot_Host <- prot_host.sub.unique.means %>%
  ggplot(aes(x = factor(Timepoint, level = level_order), y = prot_mg.cm2, color = mtORF, shape=mtORF, group = mtORF)) +
...
# Plot all data points with mean ± se
prot_plot_Sym <- data %>%
  ggplot(aes(x = factor(Timepoint.x, level = level_order), y = Sym_prot_mg.cm2, color = mtORF.x, shape=mtORF.x, group = mtORF.x)) +
  labs(x = "", y = expression("Symbiont Soluble Protein mg"~cm^{-2})) +
  geom_jitter(width = 0.3) +                                            # Plot all points
  stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1),    # Plot standard error
               geom = "errorbar", width = 0.2) +
  stat_summary(fun = mean, geom = "point", size=3) +           # Plot mean
  scale_color_manual(name="mtORF.x", values=c("orange", "cyan"))+
...
```{r}
 (prot_plot_Holo+prot_plot_Host+prot_plot_Sym) 
  ggsave("output/Poc_TPC_Protein_Allpartner_1a3b.pdf", plot = last_plot(), width = 14, height = 4)
  ggsave("output/Poc_TPC_Protein_Allpartner_1a3b.jpg", plot = last_plot(), width = 14, height = 4)
```
