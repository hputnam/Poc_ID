...
## Import data
```{r, warning = FALSE}
Respdata <- read.csv("data/May_Resp_extracted_rates.csv")
```
...
ggsave("output/May_TPC_SharpSchool_Respration.pdf", TPC.plot, dpi=300, w=8, h=8, units="in")
```
...
meandrina.ci_extra_params$Treatment <- "Haplotype10"
```
...
ggsave("output/May_TPC_estimates_SharpSchool_Respiration.pdf", estimate.plots, dpi=300, w=6, h=2, units="in")
``` 
...
ggsave("output/May_Respiration_TPC_and_estimates.pdf", figure, dpi=300, w=6, h=8, units="in")
