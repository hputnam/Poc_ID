...
## Import data
```{r, warning = FALSE}
path.p <- "data/PR/May_TPC" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "data/PR/May_TPC/May_PR_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/PR/May_TPC/May_PR_run_metadata.csv")
...
sa <- read_csv(file = "data/May_TPC_surface.area.calc.csv")
...
Photodata$Season <- "May"

# Visualize data following outlier removal
pdf("output/May_Outlier_Removed_Photo_TPC.pdf", width=6, height=6)
P_plot<-Photodata %>%
    ggplot(., aes(x = Temp.Cat, y = micromol.cm2.h, colour=mtORF)) +
    geom_point(aes(fill=mtORF, group=Number), pch = 21, size=2, alpha=0.3) + 
    xlab("Temperature") + 
...
write.csv(Photodata, file="data/May_Photo_extracted_rates.csv")
...
# Visualize data following outlier removal
pdf("output/May_Outlier_Removed_Resp_TPC.pdf", width=6, height=6)
R_plot<-Respdata %>%
    ggplot(., aes(x = Temp.Cat, y = -micromol.cm2.h, colour=mtORF)) +
    geom_point(aes(fill=mtORF, group=Number), pch = 21, size=2, alpha=0.3) + 
    xlab("Temperature") + 
...
write.csv(Respdata, file="data/May_Resp_extracted_rates.csv")
