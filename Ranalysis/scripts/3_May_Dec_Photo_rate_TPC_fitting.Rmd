...
## Import data
```{r, warning = FALSE}
Photodata_May <- read.csv("data/May_Photo_extracted_rates.csv")
Photodata_Dec <- read.csv("data/Dec_Photo_extracted_rates.csv")
Photodata <- rbind(Photodata_May,Photodata_Dec)
...
Photodata$transformed.rate <- Photodata$micromol.cm2.h
...
grandis.CI.plot.dec <- ggplot() +
  geom_line(aes(temp, .fitted), grandis.preds.dec, col = 'orange') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), grandis.boot1_conf_preds.dec, fill = 'orange', alpha = 0.3) +
  geom_point(aes(temp, transformed.rate), d.grandis.dec, size = 2, alpha = 0.5,col = 'orange') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Log 10 +1 Rate (µmol O2/cm2/h)')
grandis.CI.plot.dec 
...
verrucosa.CI.plot.dec <- ggplot() +
  geom_line(aes(temp, .fitted), verrucosa.preds.dec, col = 'cyan') +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), verrucosa.boot1_conf_preds.dec, fill = 'cyan', alpha = 0.3) +
  geom_point(aes(temp, transformed.rate), d.verrucosa.dec, size = 2, alpha = 0.5,col = 'cyan') +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (ºC)',
       y = 'Log 10 +1 Rate (µmol O2/cm2/h)')
...
```{r}
 #set plot colors
cols <- c("grandis.May"="orange","grandis.Dec"="orangered",  "verrucosa.May"="aquamarine",  "verrucosa.Dec"="cyan")
...
TPC.plot <- ggplot(data=Photodata, aes(x=temp)) +
   ...
   geom_line(aes(temp, .fitted), grandis.preds.may, col = 'orange', size=2,linetype="dotted") +
   geom_line(aes(temp, .fitted), grandis.preds.dec, col = 'orangered', size=2) +
   geom_line(aes(temp, .fitted), verrucosa.preds.may, col = "aquamarine", size=2,linetype="dotted") +
   geom_line(aes(temp, .fitted), verrucosa.preds.dec, col = "cyan", size=2) +
   geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), grandis.boot1_conf_preds.may, fill = "orange", alpha = 0.3) +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), grandis.boot1_conf_preds.dec, fill = "orangered", alpha = 0.3) +

  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), verrucosa.boot1_conf_preds.may, fill = 'aquamarine', alpha = 0.3) +
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), verrucosa.boot1_conf_preds.dec, fill = 'cyan', alpha = 0.3) +
   xlim(21,37)+
  #ylim(0,0.45)+
...
ggsave("output/Dec_TPC_SharpSchool_Photosynthesis.jpg", TPC.plot, dpi=300, w=8, h=8, units="in")
```
...
grandis.ci_extra_params.may$Treatment <- "P. grandis May"
```
#GRANDIS December
```{r}
#calculate all the TPC parameters
...
grandis.ci_extra_params.dec$Treatment <- "P. grandis December"
```
...
verrucosa.ci_extra_params.may$Treatment <- "P. verrucosa May"
```
...
verrucosa.ci_extra_params.dec$Treatment <- "P. verrucosa December"
```
...
#order levels
All_params$Treatment <- factor(All_params$Treatment, levels = c("P. grandis May", "P. verrucosa May", "P. grandis December", "P. verrucosa December"))
All_params$Treatment


#Plot all of the estimates
estimate.plots <- ggplot(All_params, aes(Treatment, estimate, color=Treatment)) +
  geom_point(size = 2) +
  scale_color_manual(name="Treatment", values=c("orange","aquamarine", "orangered","cyan"))+
...
ggsave("output/Dec_TPC_estimates_SharpSchool_Photosynthesis.jpg", estimate.plots, dpi=300, w=6, h=2, units="in")
``` 
...
ggsave("output/MayDec_Photosynthesis_TPC_and_estimates.pdf", figure, dpi=300, w=6, h=8, units="in")
ggsave("output/MayDec_Photosynthesis_TPC_and_estimates.jpg", figure, dpi=300, w=6, h=8, units="in")
